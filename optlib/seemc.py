...
class Electron:
    ...
    def print_info(self):
        ...
        print("Pathlength:",self.path_length)
    ...
    def update_direction(self,uvw,deflection,algorithm=0):
        ...
        if algorithm == 0:
            ...
            if abs(uvw[2]) > 0.99:
                ...
                base_uvw[0] = uvw[0]*math.cos(deflection[0]) + sq1/sqw*(uvw[0]*uvw[2]*math.cos(deflection[1]) - uvw[1]*math.sin(deflection[1]))
                base_uvw[1] = uvw[1]*math.cos(deflection[0]) + sq1/sqw*(uvw[1]*uvw[2]*math.cos(deflection[1]) + uvw[0]*math.sin(deflection[1]))
            ...
            theta = math.acos(math.cos(theta_old)*math.cos(deflection[0]) - math.sin(theta_old)*math.sin(deflection[0])*math.cos(deflection[1]))
    ...
    def run_simulation(self,plot_yields = False):
        ...
        for energy in self.energy_array:
            ...
            for traj in tqdm(range(self.n_trajectories)):
                e_statistics.append(Electron(self.sample,energy,self.cb_ref,self.track_trajectories,[0,0,0],[math.sin(self.incident_angle),0,math.cos(self.incident_angle)],0,False,-1))
                        ...
                        if e_statistics[i].inside and not e_statistics[i].dead:
                            ...
                            if e_statistics[i].scatter():
                                ...
                                if se_energy > e_statistics[i].inner_potential:
                                    ...
                                    se_uvw = e_statistics[i].update_direction(e_statistics[i].uvw,[math.asin(math.cos(e_statistics[i].deflection[0])),e_statistics[i].deflection[1] + math.pi])
                                    e_statistics[i].n_secondaries += 1
                                    e_statistics.append(Electron(self.sample,se_energy,self.cb_ref,self.track_trajectories,se_xyz,se_uvw,e_statistics[i].generation + 1,True,i))
                    i += 1
